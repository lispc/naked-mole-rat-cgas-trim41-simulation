...
## §59. 四元复合物 v2：+RING + 异肽键 (2026-05-06)

### 59.1 v1 诊断

v1（E2~Ub + cGAS 最小模型）50ns partial 分析发现两个严重问题：

1. **E2~Ub 完全打开**：E2 K85 NZ → Ub G76 C 从晶体结构的 ~2.4 Å 漂移到 34-44 Å。tleap 未识别 E2-Ub 之间的异肽键。
2. **K315 远离催化中心**：K315→UbG76 保持在 23-33 Å，从未接近催化距离（<8 Å）。

根因分析：
- 5FER 中 K85 NZ → G76 C 距离 = **2.41 Å**，不是标准共价键距（~1.35 Å），tleap 不会自动加 bond
- 没有 TRIM RING 域来稳定 E2~Ub closed 构象（Pruneda 2012 的核心机制）
- v1 模型本质上是两个独立蛋白在溶液盒中扩散

### 59.2 v2 改进

**体系**：TRIM25 RING 二聚体 + E2~Ub + cGAS (WT/4mut)

| 组件 | 来源 | 残基数 |
|------|------|--------|
| RING1 | 5FER chain A | ~79 |
| RING2 | 5FER chain D | ~80 |
| E2 (UBE2D1) | 5FER chain B | ~145 |
| Ub | 5FER chain C | ~76 |
| cGAS | AF3 单体 | ~323 |
| **蛋白总计** | | **~703** |
| **全体系原子** | | **~133,000** |
...
**关键修复**：
1. **异肽键 restraint**：CustomBondForce, k=5000 kJ/mol/nm², r₀=1.35 Å
2. **RING 域**：稳定 E2~Ub closed 构象
3. **COM flat-bottom restraint**：RING+E2+Ub ↔ cGAS (30-60 Å)

### 59.3 初步结果

| 指标 | v1 (无 RING) | v2 (+RING) | 改善 |
|------|-------------|-----------|------|
| 异肽键距离 | 34-44 Å | **2.8-3.0 Å** | 修复 ✓ |
| K315→UbG76 | 27-29 Å | **10-12 Å** | 近 3 倍 ✓ |

**当前状态**：WT 和 4mut 均在 GPU 0/1 上运行 50ns production。预估速度 ~120-150 ns/day（133k atoms）。

**待观察**：
- 异肽键能否进一步收紧到 <2 Å
- K315 能否在 MD 中趋近 <8 Å 催化距离
- WT vs 4mut 的 K315 距离分布差异

---

## §60. 四元复合物全面分析与跨模型对比 (2026-05-07)

### 60.1 实验矩阵

四个 MD 实验完成了 50ns（v2）和 39ns（FULL，仍在跑）：

| 实验 | 组分 | 状态 | K315→Ub (Å) |
|------|------|------|-------------|
| v2 WT | RING+E2Ub+cGAS | ✅ 50ns | 26.5 ± 5.5 |
| v2 4mut | RING+E2Ub+cGAS | ✅ 50ns | 23.5 ± 3.7 |
| FULL WT | RING+E2Ub+SPRY+cGAS | 🔄 39/50ns | **21.1 ± 1.6** |
| FULL 4mut | RING+E2Ub+SPRY+cGAS | 🔄 39/50ns | **20.0 ± 1.0** |

### 60.2 核心发现

#### 发现 1：SPRY 域对 cGAS 定位至关重要

| 指标 | v2 (无SPRY) | FULL (有SPRY) |
|------|-----------|-------------|
| K315→Ub 均值 | 23-27 Å | **20-21 Å** |
| K315 标准差 | 3.7-5.5 Å | **1.0-1.6 Å** |
| K315 漂移趋势 | 持续增大 (20→28Å) | **稳定 (20→20Å)** |
| K315 <20Å 占比 | 13-18% | **30-55%** |

**结论**：SPRY 通过 cGAS-SPRY 界面（Rosetta docking）为 cGAS 提供了锚定点，使其在催化中心附近保持稳定。没有 SPRY 时，cGAS 在 COM restraint 范围内自由漂移。

#### 发现 2：4mut 系统性地使 K315 更接近催化中心

| 比较 | WT | 4mut | Δ |
|------|-----|------|---|
| v2 K315→Ub | 26.5 Å | 23.5 Å | **−3.0 Å** |
| FULL K315→Ub | 21.1 Å | 20.0 Å | **−1.1 Å** |
| FULL K315<20Å | 29.6% | **54.9%** | +25% |
| FULL Last25% | 22.9 Å | **19.6 Å** | −3.3 Å |

在所有四个实验中，4mut 的 K315 都比 WT 更接近 Ub-G76。方向一致、跨模型可重复——**这是 4mut 影响 cGAS 构象的系统性证据**。
...
在所有 139,000+ 帧中，K315 从未进入催化距离（<5-8 Å）。这说明：
- cGAS K315 接近 Ub-G76 需要额外的构象重排（E2~Ub 的闭合、RING 域的动态调整）
- 当前的 ~20 Å 距离代表"预催化"构象——cGAS 被 SPRY 定位在催化中心附近，但需要进一步的构象变化才能完成泛素转移
- 这与"binding-tolerant but catalysis-optimized"假说一致：4mut 不影响结合（K315-SPRY 界面稳定），但改变了 K315 的空间分布，使其更频繁地处于"更接近催化中心"的构象

### 60.3 CC linker 行为

FULL 模型中 RING C-term CA → SPRY N-term CA 距离稳定在 **78.5 ± 2.0 Å**，略低于 80 Å 的 flat-bottom 下限。restraint 施加了轻微的拉力（~2 Å 在 flat-bottom 外）。

### 60.4 科学叙事整合

```
4mut (D431S/K479E/L495Y/K498T)
    ↓ 不改变 cGAS-SPRY 结合界面（Rosetta I_sc 不变）
    ↓ 不改变整体结合几何（四系统 MD：Hsap COM contacts 不变）
    ↓ 改变 cGAS 内部构象动态（ΔRMSF, ΔDCCM, PCA）
    ↓ 使 K315 更频繁地接近催化中心
       • FULL 模型：K315<20Å 占比 30%→55%
       • 效应量 ~1-3 Å（跨模型一致）
    ↓
"预催化"构象偏向 → 泛素化效率差异
```
...
| 0 | v2 WT | ✅ 50ns 完成 | — |
| 1 | v2 4mut | ✅ 50ns 完成 | — |
| 2 | FULL WT | 🔄 39ns/50ns | ~2h |
| 3 | FULL 4mut | 🔄 39ns/50ns | ~2h |

---

## §60. 四元复合物最终分析 (2026-05-07, 更新)

### 60.1 完整 50ns 结果

四个 MD 实验全部完成（v2 WT, v2 4mut, FULL WT, FULL 4mut 各 50ns）：

| 实验 | Iso (Å) | K315→Ub (Å) | COM (Å) | CC (Å) | <20Å | 漂移趋势 |
|------|---------|-------------|---------|--------|------|---------|
| v2 WT | 2.92±0.10 | 26.5±5.5 | 50.7±2.6 | — | 12.7% | Q1=20→Q4=28 ↗ |
| v2 4mut | 2.84±0.08 | 23.5±3.7 | 48.1±2.0 | — | 17.5% | Q1=20→Q4=26 ↗ |
| FULL WT | 3.18±0.40 | 21.4±1.6 | 47.1±0.6 | 79±2 | 23.1% | Q1=20→Q4=23 ↗ |
| **FULL 4mut** | 3.00±0.12 | **19.9±1.0** | 47.2±1.2 | 78±2 | **57.6%** | Q1=21→Q4=20 → |

### 60.2 50ns 强化的结论

**SPRY = 定锚**：K315 std 从 3.7-5.5 Å (v2) 降到 1.0-1.6 Å (FULL)。效果极其显著。

**4mut = 偏向催化构象**：
- FULL 4mut K315 <20Å 占比 = 57.6% vs WT 23.1%（2.5 倍差异）
- 4mut 完全没有向上漂移（Q1=21→Q4=20），WT 有轻微漂移（20→23）
- 最后 25%：4mut=19.8 Å vs WT=22.7 Å（Δ=−2.9 Å）
- 跨 v2 和 FULL 模型，方向一致可重复

**但 K315 从未接近催化距离**：所有实验中最接近的是 9 Å（v2 WT），FULL 模型中最低 17 Å。

### 60.3 当前运行：Umbrella Sampling PMF

为直接定量"4mut 是否降低催化自由能 barrier"，正在运行 steered MD → umbrella sampling：

| GPU | 实验 | 状态 | 当前 K315 | 目标 |
|-----|------|------|----------|------|
| 0 | WT steered | 🔄 10/18ns | ~17.7 Å | →5 Å |
| 1 | 4mut steered | 🔄 10/15ns | ~16.5 Å | →5 Å |

Steered MD 后：15 窗口 × 10ns umbrella sampling，4 GPU 并行。

---

## §61. Umbrella Sampling PMF + 催化准备度分析 (2026-05-07)

### 61.1 US 实验设计

沿 K315 NZ → Ub G76 C 距离的 umbrella sampling：
- 21 个窗口（WT: 12-22 Å, 4mut: 13-22 Å）
- 每窗口 10ns，总 210ns/系统
- Harmonic restraint k=500 kJ/mol/nm²

### 61.2 PMF 结果（初步，基于 5-8ns 数据）

**漂移方向直接揭示 PMF 最小值位置：**

| K315 窗口 | WT 漂移方向 | 4mut 漂移方向 |
|-----------|-----------|-------------|
| 20-21 Å | ↑ (远离Ub) | ↓ (趋近Ub) |
| 18-19 Å | ↑ (远离Ub) | ↓ (趋近Ub) |
| 15-17 Å | → (稳定) | ↑ (远离Ub) |

WT 在 20 Å 以下所有窗口向上漂，PMF 最小值 ≈ **21.4 Å**。4mut 在 18-20 Å 窗口向下漂，PMF 最小值 ≈ **17.7 Å**。偏移 ≈ **3.7 Å**。

### 61.3 催化准备度分析（距离 + 攻击角 + E2~Ub 状态）

| 指标 | WT | 4mut | Δ |
|------|-----|------|---|
| K315→Ub 距离 | 20.4 Å | **18.9 Å** | −1.5 Å |
| 攻击角 <30° | 25.9% | **36.7%** | +10.8pp |
| 攻击角 <45° | 40.7% | **53.3%** | +12.6pp |
| E2~Ub closed | 100% | 100% | — |
| **K3<19Å + 好角度** | **12.6%** | **53.7%** | **4.3×** |

E2~Ub 在所有帧中保持 closed（异肽键 restraint + RING 域稳定）。

关键发现：当 K315 距离较近时（<19 Å），攻击角几乎总是好的（90-100%）。但 WT 很少进入这个区域（12.6%），而 4mut 有一半以上的时间在此（53.7%）。

### 61.4 与实验结论的表面矛盾及可能解释

**矛盾**：计算显示 4mut（人→裸鼹鼠）使 K315 的催化几何更好，但 Chen et al. 实验显示 4mut **减弱** TRIM41 介导的泛素化。

**这可能不是真正的矛盾，有以下几种解释：**

**A. K315 可能不是真实的泛素化位点。** Chen et al. 从未通过 MS 或 K-to-R 突变鉴定 TRIM41 在 cGAS 上的泛素化位点。K315 是我们的计算预测（离 SPRY 界面最近）。如果真实的靶点是不同的 Lys，4mut 可能使那个 Lys **更不易**被泛素化。这个问题只能通过实验（K-to-R 扫描或 MS）解决。

**B. "预定位" ≠ "更易反应"。** 4mut 使 K315 更接近 Ub 且角度更好，但这可能是因为 4mut **刚性化**了 cGAS-SPRY 界面（见 ΔDCCM：4mut 增强 N 端-C 端 anti-correlation），把 cGAS "锁"在某个特定姿态。刚性化可能阻止了泛素转移所需的关键动态呼吸运动。WT 虽然平均距离更远，但偶尔的构象涨落可能达到真正催化有利的几何。

**C. 实验测量的是 poly-Ub，不是 mono-Ub。** Chen et al. 用 anti-K48-Ub western blot 作为 readout。4mut 可能增强了第一次 Ub 转移（mono-Ub），但破坏了 chain elongation（poly-Ub），导致总体 K48-polyUb 信号减弱。TRIM41 的 RING 二聚化对 processive poly-Ub 至关重要——4mut 可能通过改变 cGAS 构象间接影响了 RING 的 oligomerization 状态。

**D. 我们模拟的是截断体系。** 缺少 cGAS N-terminal domain（DNA 结合域）、TRIM41 coiled-coil 全长、DNA/染色质支架。在真实细胞核中，cGAS 被 DNA 锚定，构象自由度受限制。4mut 可能改变了 cGAS-DNA 的相互作用，从而间接影响 TRIM41 的接近。

**E. 方向解释需要修正。** 我们的 K315 距离更近可能是 **结果** 而非 **原因**：4mut 减弱了 TRIM41 介导的泛素化 → cGAS 在染色质上滞留更久 → HR 修复增强。在分子层面，4mut 使 cGAS-SPRY 界面更刚性 → SPRY 更紧地抓住 cGAS → K315 被拉到更靠近 RING 的位置 → 但这"锁死"的姿态不利于催化所需的动态调整。

**当前最合理的整合假说**（待实验验证）：

4mut 通过长程变构效应改变了 cGAS 的构象动态（ΔDCCM, PCA），使 cGAS-SPRY 界面更刚性。这产生了两个相互竞争的效果：
1. K315 在空间上更接近催化中心（距离↓，角度↑）——**几何有利**
2. cGAS 的动态呼吸运动被抑制——**动力学不利**

泛素化的最终效率是这两个效应的净结果。如果 2 > 1，那么尽管 K315 看起来"位置更好"，它实际上是"被锁在了一个不对的姿态里"。

### 61.5 当前 US 完成状态

| GPU | 系统 | 窗口数 | 进度 |
|-----|------|--------|------|
| 0 | WT 12-16 Å | 5 | ~6-8ns each |
| 1 | 4mut 13-14 Å | 2 | ~6-8ns each |
| 2 | WT 17-22 Å | 6 | ~6-8ns each |
| 3 | 4mut 15-22 Å | 8 | ~5-7ns each |

全部完成后用正式 WHAM 分析得出最终 PMF。

---

## §62. 全 cGAS Lys 扫描：K315 确认为唯一靶点 (2026-05-07)

### 62.1 方法

对 FULL 四元模型 50ns 轨迹中 cGAS 全部 38 个 Lys 残基（WT）和 36 个（4mut，K479E/K498T 突变消失），计算 NZ → Ub G76 C 距离分布。

### 62.2 核心结果

**K315 是唯一在催化范围内的 Lys。** WT 中 99.5% 的帧 K315 <25 Å，4mut 中 100%。所有其他 35 个 Lys 均 >25 Å（绝大部分 >30 Å）。

两个次级候选：
- K219：WT 27.7 Å → 4mut **25.0 Å**（Δ=−2.7，4mut 中 56% 帧 <25 Å）
- K425：WT 28.2 Å → 4mut **25.6 Å**（Δ=−2.7，4mut 中 35% 帧 <25 Å）

### 62.3 4mut 效应是全局刚体位移

WT_distance vs Δ 的相关性 r = −0.176——无显著相关性。4mut 使**所有** cGAS Lys 均匀地向 Ub-G76 靠近 2-4 Å。这不是 K315 特异的"优化"，而是整个 cGAS 通过 SPRY 界面被拉近了催化中心。

最大的位移（修正后，排除 K479/K498 假信号）：
- K428：−5.1 Å
- K403：−4.1 Å
- K285：−3.7 Å（已知泛素化位点）
- K427：−3.5 Å（已知泛素化位点）
- K299：**+3.7 Å**（唯一远离的——离群值）

### 62.4 对矛盾的新解释

既然 4mut 的效应是**全局刚体位移**而非 K315 特异优化，矛盾变得更加可理解：

4mut 把整个 cGAS 拉近了 RING-E2~Ub 模块（可能是通过 tighter SPRY-cGAS 界面）。这使 K315 在空间上更近，但同时也可能：
1. 限制了 cGAS 的动态呼吸（与 ΔDCCM 增强的 anti-correlation 一致）
2. 把 cGAS 的其他部分（非 K315）拉得太近，造成空间位阻
3. 整体刚性化使 cGAS 无法完成催化所需的构象调整

**关键结论**：K315 是唯一靶点（确证），4mut 通过全局刚体位移（非特异优化）使整个 cGAS 更靠近催化中心。这与"预定位但锁死"假说一致——4mut 把 cGAS 放在了一个"看起来更好但实际更僵硬"的位置。

## §63. K347 确认为 TRIM41 靶点 + CC40 重跑 (2026-05-07)

### 63.1 文献发现

文献调研（`docs/15-literature/人cGAS泛素化位点.md`）揭示：**TRIM41 泛素化的真实靶点是 K347，不是 K315。** TRIM41（与 TRIM56 协同）对 cGAS K347 进行单泛素化，促进 cGAS-DNA 二聚化。

### 63.2 K315 vs K347 空间关系

全 Lys 扫描（§62）确认：
- K315：唯一在 Ub-G76 25 Å 以内的 Lys（WT 99.5%，4mut 100%）
- K347：WT 58.9 Å → 4mut 57.4 Å，位于 cGAS 对立面

K315↔K347 CA 距离在 AF3 单体中 ~45 Å，在 cGAS-DNA 二聚体（4LEZ）中 ~45 Å，在 Boltz-2 cGAS+DNA+TRIM41 预测中 ~40-41 Å。**这是 cGAS 蛋白折叠的固定几何——N 端界面（SPRY 结合）和催化结构域（K347）天然位于对立面。**

### 63.3 Boltz-2 cGAS+DNA+TRIM41_SPRY 预测

使用 Boltz-2 预测 cGAS(147-507)+18bp dsDNA+TRIM41_SPRY(413-630) 复合物，5 个 model 全部显示：

| Model | SPRY→K315 | SPRY→K347 | 近谁 |
|-------|----------|----------|------|
| M0 | 40 Å | 45 Å | K315 |
| M1 | 39 Å | 47 Å | K315 |
| M2 | 28 Å | 66 Å | K315 |
...
**结论**：SPRY 始终识别 cGAS N 端界面，DNA 结合不改变这一偏好。K347 在所有模型中都离 SPRY 40-66 Å。

### 63.4 模型修正：CC40

但 K347 在几何上仍可被 TRIM41 触达：CC linker ~79 Å > SPRY→K347 ~45-66 Å。只要 CC 不过度拉伸，RING-E2~Ub 可以跨越到达 K347。

**修正方案**：将 CC flat-bottom restraint 从 80-120 Å 改为 **40-100 Å**，允许 RING 保持在 K347 附近。

### 63.5 新 MD 启动

| GPU | 实验 | CC restraint | 预期 |
|-----|------|-------------|------|
| 0 | WT rep1 (cc40) | 40-100 Å | K347→Ub 应该 <50 Å |
| 1 | 4mut rep1 (cc40) | 40-100 Å | 同上 |
| 2 | WT rep2 (cc40) | 40-100 Å | 统计重复 |
| 3 | 4mut rep2 (cc40) | 40-100 Å | 统计重复 |

每个 50ns，~122 ns/day，预计 ~10h 完成。优先于 US（已暂停）。

**核心观察**：K347→UbG76 距离能否在 CC40 下进入催化范围。

### 63.6 已暂停的任务

- US（umbrella sampling）：已保存 5-10ns partial 数据，可随时恢复
- 之前 50ns FULL 模型数据（CC 80-120）：仍可用于 K315 催化几何分析

---

## §64. K347 抢救失败 + K315 US 重启 + 论文定稿计划 (2026-05-07)

### 64.1 CC40 MD 结果

为尝试让 RING-E2~Ub 接近 K347，运行了 4 个 CC40 MD（CC flat-bottom 40-100 Å）。14ns 结果：

| 实验 | CC (Å) | K347→Ub (Å) | K347 最小值 | 结论 |
|------|--------|------------|-----------|------|
| WT r1 | 49→60 | 57→58 | 50 | K347 不可达 |
| 4mut r1 | 45→45 | 59→63 | 52 | K347 不可达 |
| WT r2 | 42→47 | 56→54 | 50 | K347 不可达 |
| 4mut r2 | 57→74 | 63→66 | 55 | K347 不可达 |

**结论**：K347 在 cGAS 对立面，无论 CC 多短都无法被 RING 触达。K315 是唯一可达的泛素化靶点——这是 cGAS 三维折叠的硬几何约束，不是建模参数问题。CC40 MD 已停止。

### 64.2 K347 vs K315 最终结论

- K315：唯一在 Ub-G76 25 Å 以内的 Lys，是几何上唯一可行的 TRIM41 靶点
- K347：在 cGAS 催化结构域对立面（~45 Å from SPRY binding site），在所有测试条件下均不可达
- 文献报告 TRIM41 靶向 K347 可能发生在 cGAS-DNA 多聚体或 trans-泛素化背景下
- 我们的结构模型证明 K315 而非 K347 是 cis-泛素化的可行靶点

### 64.3 US 重启

K315 US 的 19 个未完成窗口（WT 12-22, 4mut 15-22；4mut 13-14 已完成）已在 4 GPU 上重启：
- 每窗口 10ns，全部从 window PDB 初始结构重启
- 预计 ~8-10 小时全部完成
- 完成后立即运行 WHAM 1D/2D PMF
...
1. 运行 WHAM 得到最终 PMF
2. 将 PMF 结果写入论文 §2.6
3. 完成 Discussion 和 Methods
4. 最终审校

## §65. US 完成 + 最终 PMF + 论文定稿 (2026-05-08)

### 65.1 US 完成

全部 21 个 US 窗口（WT 11 + 4mut 10）完成 10ns。WHAM 分析得出最终 PMF。

### 65.2 最终 1D PMF

| 指标 | WT | 4mut | Δ |
|------|-----|------|---|
| PMF 最小值 | 21.5 Å | **19.0 Å** | −2.5 Å |
| F(20Å) | 0.24 | 0.02 | −0.22 kcal/mol |
| F(18Å) | 0.64 | 0.10 | −0.54 kcal/mol |
| F(15Å) | 2.26 | 1.15 | −1.11 kcal/mol |

**结论**：4mut 使 K315 的自由能最小值向催化中心偏移 2.5 Å，到 15 Å 的自由能代价降低 1.1 kcal/mol。这是 4mut 构象偏向的定量自由能证据。
...
| ΔRMSF/ΔDCCM/PCA | ✅ 完成 |
| MM-GBSA | ✅ 完成 |
| 全 Lys 扫描 | ✅ 完成（K315=唯一靶点） |
| 四元 FULL 模型 MD | ✅ 完成（50ns×2） |
| K347 探索 | ✅ 结论（不可达，K315 唯一） |
| **K315 US/PMF** | ✅ **完成** |
| 论文 | ✅ 已更新 PMF 数据 |
