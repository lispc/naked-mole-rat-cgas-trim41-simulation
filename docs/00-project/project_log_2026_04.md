...
- 报告：`docs/10-reports/rosetta_docking_report.md`
...
详见：`docs/10-reports/af3_mutation_analysis.md` 第 7-8 节
...
详见：`docs/00-project/paper_notes_cgas_trim41.md` 第 4 节（关键新发现）
...
详见：`docs/10-reports/cite28_analysis.md`
...
详见：`docs/10-reports/cite28_analysis.md` 第5-6节
...
详见：`docs/10-reports/cite28_analysis.md` 第6节
...
*最后更新：2026-05-02 ( 引用28分析 + UniProt泛素化位点 + 假说修正 )*
*维护者：Kimi Code CLI*
...
详见：`docs/10-reports/interface_analysis_report.md` 第5节
...
详见：`docs/10-reports/interface_analysis_report.md` 第6节
...
| 文件 | 类型 | 描述 |
|------|------|------|
| `docs/10-reports/interface_analysis_report.md` | 报告 | 完整界面分析报告 |
| `docs/00-project/summary_2026-04-23.md` | 日报 | 今日工作总结 |
...
| `docs/00-project/4mut_correction_log.md` | 完整修正记录 |
...
*最后更新：2026-05-02 ( 4mut 位点紧急修正 + 论文原文确认 )*
*维护者：Kimi Code CLI*
...
编写 `docs/00-project/proposed_validation_experiments.md`，列出 10 个计算实验方案，按 P0/P1/P2 优先级排序：
...
| `docs/10-reports/rosetta_mutational_scan_report.md` | 突变扫描报告 |
| `docs/10-reports/rosetta_docking_4mut_report.md` | 对接验证报告 |
| `docs/00-project/proposed_validation_experiments.md` | 验证方案 |
...
详见独立文档 `docs/30-diagnostics/md_realism_gap.md`。核心差距：
...
## 四十七、MM-GBSA 结果修正与过度解读纠正（2026-05-02）

### 47.1 此前表述的问题

在 §45.5 和 §46.4 中，对 Hsap_4mut MM-GBSA 结果的解读存在**过度推断**：
...
**MM-GBSA 精度限制**（文献：Genheden & Ryde, 2015, *J. Chem. Inf. Model.*）：
...
1. **RMSF 差异热图**：WT vs 4mut 的 cGAS 各区域柔性对比
2. **RING-Lys 距离 PMF**：计算 TRIM41 RING → cGAS-Lys315 (topology resid 334) 的自由能面
3. **三元复合物建模**：AF3/Boltz-2 预测 TRIM41 RING + E2(UbcH5b) + Ub + cGAS
4. **DCCM 差异分析**：识别 4mut 引起的变构路径变化
...
1. **体外泛素化实验（in vitro ubiquitination assay）**
   ...
   - 预期：4mut 泛素化水平低于 WT，但 co-IP 结合可能不变

2. **HDX-MS（氢氘交换质谱）**
   - 比较 cGAS(WT) vs cGAS(4mut) 的构象动态
   - 重点看 N-terminal 界面区域（res 211–219）和催化相关 loop

3. **体外 E3 活性动力学**
   - 固定 TRIM41，梯度增加 cGAS(WT/4mut)
   - 测定 V_max 和 K_m
   - 若 K_m 不变但 V_max 降低 → 支持 "binding OK, catalysis impaired"

4. **FRET 实时监测**
   - 在 TRIM41 RING 和 cGAS Lys315 (topology resid 334) 区域标记荧光探针
   - 预期：WT RING-Lys 距离分布更窄（催化有利），4mut 更宽（催化不利）
