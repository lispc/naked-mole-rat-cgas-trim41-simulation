"""Corrected comparison using align.alignto() for proper alignment."""
import os
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
def analyze(u, ref_u, label, step=1):
    ...
    for i, idx in enumerate(indices):
        ...
        # Use align.alignto for proper Kabsch alignment
        # This modifies u atoms in-place to align to ref_u
        rmsd_val = align.alignto(u, ref_u, select="protein and name CA", weights="mass")[1]
        
        trim41 = u.select_atoms("protein and resid 1-218")
        cgas = u.select_atoms("protein and resid 219-541")
        prot = u.select_atoms("protein")
        
        # COM distance (after alignment)
...
print("\nGROMACS (whole xtc, align.alignto)...")
gmx_data = analyze(u_gmx, ref_gmx, "GROMACS", step=10)

print("\nOpenMM...")
omm_data = analyze(u_omm, ref_omm, "OpenMM", step=1)

for name, data in [("gmx_v4", gmx_data), ("omm_v4", omm_data)]:
...
ax4.set_title('Protein CA RMSD (align.alignto)')
...
ax5.set_xlabel('Time (ns)'); ax4.set_ylabel('RMSD (Å)')
ax5.set_title('cGAS CA RMSD')
ax5.legend(); ax5.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTDIR, 'gmx_vs_openmm_200ns_v4.png'), dpi=150)
print(f"\nPlot saved.")

print("\n=== 200ns Statistics (Corrected) ===")
